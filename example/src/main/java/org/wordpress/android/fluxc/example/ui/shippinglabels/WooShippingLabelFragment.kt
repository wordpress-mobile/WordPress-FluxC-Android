...
import org.wordpress.android.fluxc.example.databinding.FragmentWooShippinglabelsBinding
...
class WooShippingLabelFragment : StoreSelectingFragment() {
    ...
    ): View = FragmentWooShippinglabelsBinding.inflate(inflater, container, false).root

    @Suppress("LongMethod", "ComplexMethod", "SwallowedException", "TooGenericExceptionCaught")
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        with(FragmentWooShippinglabelsBinding.bind(view)) {
            fetchShippingLabels.setOnClickListener {
            ...
            refundShippingLabel.setOnClickListener {
                selectedSite?.let { site ->
                    showSingleLineDialog(activity, "Enter the order ID:", isNumeric = true) { orderEditText ->
                        if (orderEditText.text.isEmpty()) {
                            prependToLog("OrderId is null so doing nothing")
            ...
            printShippingLabel.setOnClickListener {
                selectedSite?.let { site ->
                    showSingleLineDialog(
            ...
            printCommercialInvoice.setOnClickListener {
                selectedSite?.let { site ->
                    coroutineScope.launch {
            ...
            checkCreationEligibility.setOnClickListener {
                selectedSite?.let {
                    coroutineScope.launch {
            ...
            createCustomPackage.setOnClickListener {
                selectedSite?.let { site ->
                    coroutineScope.launch {
            ...
            activatePredefinedPackage.setOnClickListener {
            ...
            getShippingRates.setOnClickListener {
                selectedSite?.let { site ->
                    coroutineScope.launch {
            ...
            purchaseLabel.setOnClickListener {
                selectedSite?.let { site ->
                    coroutineScope.launch {
                        val orderId = showSingleLineDialog(requireActivity(), "Enter order id:", isNumeric = true)
                            ?.toLong() ?: return@launch
            ...
            getShippingPluginInfo.setOnClickListener {
                selectedSite?.let { site ->
                    coroutineScope.launch {
                        val result = withContext(Dispatchers.Default) {
            ...
            getAccountSettings.setOnClickListener {
                selectedSite?.let { site ->
                    coroutineScope.launch {
