...
class ReleaseStack_ThreatTestJetpack : ReleaseStack_Base() {
    ...
    fun testFetchThreat() {
        ...
        val payload = ThreatStore.FetchThreatPayload(site, BuildConfig.TEST_THREAT_ID.toLong())

        mCountDownLatch = CountDownLatch(1)
        val fetchedThreatStatePayload = runBlocking { threatStore.fetchThreat(payload) }

        assertNotNull(fetchedThreatStatePayload)
        assertEquals(
            BuildConfig.TEST_THREAT_ID.toLong(),
            requireNotNull(fetchedThreatStatePayload.threatModel?.baseThreatModel).id
        )
    }
