...
class ReleaseStack_WooPaymentsTest : ReleaseStack_WCBase() {
    ...
    fun whenFetchingDepositOverviewThenDataReturned() = runBlocking {
        ...
        assertEquals(0L, result.result?.balance?.pending?.get(0)?.amount)
        ...
        assertEquals(4373L, result.result?.deposit?.lastPaid?.get(0)?.amount)
    ...
    fun whenSavingDepositOverviewAndGetThenDataReturned() = runBlocking {
        ...
        assertEquals(0L, getResult?.balance?.pending?.get(0)?.amount)
        ...
        assertEquals(4373L, getResult?.deposit?.lastPaid?.get(0)?.amount)
