...
@Inject constructor(
    ...
    fun livePostAndPageViews(
        site: SiteModel,
        granularity: StatsGranularity,
        cacheMode: LimitMode,
        date: Date
    ): LiveData<PostAndPageViewsModel> {
        return sqlUtils.liveSelectPostAndPageViews(site, granularity, date).map { timeStatsMapper.map(it, cacheMode) }
    }
}
