...
class MigrationSchemasTests {
    companion object {
        const val SCHEMAS_FOLDER = "org.wordpress.android.fluxc.persistence.WCAndroidDatabase"
        const val HASH_TAG_NAME = "identityHash"
        const val DATABASE_TAG_NAME = "database"

        private fun getDBHashKey(dbVersion: Int):String = "DB_V${dbVersion}_HASH"
    ...
    fun testDBHashes() {
        for(i in 3..WC_DATABASE_VERSION){
            val currentHash = getIdentityHash(i)
            val expectedHash = DB_HASHES.getValue(getDBHashKey(i))
            assertThat(currentHash).isEqualTo(expectedHash)
        }
    }

    private fun getIdentityHash(dbVersion: Int): String {
